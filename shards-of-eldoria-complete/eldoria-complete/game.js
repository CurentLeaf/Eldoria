...
function Game() {
    ...
    // "story" = in story nodes, "free" = village / free roam
    this.mode = "story";
}
...
Game.prototype.displayCurrentNode = function () {
    ...
    // Special marker node that hands control to free‑roam UI
...
Game.prototype.makeChoice = function (idx) {
    ...
    // Special UI actions (shop, fishing, gambling, etc.)
    if (choice.action) {
...
Game.prototype.afterCombat = function (victory, fled) {
    ...
    if (this.pendingChoice && this.pendingChoice.next) {
        // Story fight: continue story
        this.currentNode = this.pendingChoice.next;
        this.pendingChoice = null;
        this.displayCurrentNode();
    } else {
        // Random encounter (from explore / fishing, etc.)
        this.openMain();
    }
...
Game.prototype.explore = function (locationId) {
    ...
    if (!location.accessible) {
        this.ui.showMessage("This area is not yet accessible.");
        return;
    }
    ...
    if (location.enemies && location.enemies.length > 0) {
        var enemyId =
            location.enemies[
                Math.floor(Math.random() * location.enemies.length)
            ];
        this.pendingChoice = null;
...
Game.prototype.buyItem = function (itemId, price) {
    ...
    if (this.character.inventory.length >= this.character.maxInventory) {
    ...
    var itemData = ITEMS[itemId] || WEAPONS[itemId] || {};
    this.ui.showNotification(
        "Purchased " + (itemData.name || itemId) + "!"
    );
...
Game.prototype.castLine = function () {
    ...
    if (result.legendary && result.combat) {
        var self = this;
        setTimeout(function () {
            self.startCombat(result.combat);
        }, 2000);
    }
...
Game.prototype.playGamblingGame = function () {
    ...
    var bet = betInput ? parseInt(betInput.value, 10) : 10;

    var gameData = GAMBLING_GAMES[this.currentGamblingGame];
    if (!gameData) return;

    var startResult = this.gambling.startGame(
        this.currentGamblingGame,
        null,
        bet
    );
    if (!startResult.success) {
        this.ui.showMessage(startResult.message);
        return;
    }
...
// Base navigation helpers
Game.prototype.openMain = function () {
    this.mode = "free";
    this.ui.showMainInterface();
};

Game.prototype.goBack = function () {
    // From overlays: go back to whichever base mode we were last in
    if (this.mode === "story") {
        this.displayCurrentNode();
    } else {
        this.openMain();
    ...
    try {
        var saveData = {
            ...
            fishing: {
                ...
                totalCatches: this.fishing.totalCatches,
                equipped: this.fishing.equipped
            },
            gambling: {
                gamesPlayed: this.gambling.gamesPlayed,
                totalWon: this.gambling.totalWon,
                totalLost: this.gambling.totalLost
            }
        };
...
Game.prototype.loadGame = function () {
    try {
        ...
        this.character = new Character(
            ...
            saveData.character.difficulty
        );
        Object.assign(this.character, saveData.character);

        this.currentNode = saveData.currentNode || "opening";
        this.mode = saveData.mode || "story";

        if (saveData.quests) {
            this.quests.activeQuests =
                saveData.quests.activeQuests || {};
            this.quests.completedQuests =
                saveData.quests.completedQuests || [];
        }
        if (saveData.fishing) {
            this.fishing.fishCaught =
                saveData.fishing.fishCaught || {};
            this.fishing.totalCatches =
                saveData.fishing.totalCatches || 0;
            this.fishing.equipped =
                saveData.fishing.equipped || {
                    rod: "basic_rod",
                    bait: null
                };
        }
        if (saveData.gambling) {
            this.gambling.gamesPlayed =
                saveData.gambling.gamesPlayed || 0;
            this.gambling.totalWon =
                saveData.gambling.totalWon || 0;
            this.gambling.totalLost =
                saveData.gambling.totalLost || 0;
        }
