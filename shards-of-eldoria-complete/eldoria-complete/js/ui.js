...
GameUI.prototype.displayStoryNode = function(node) {
    if (node.text === "SHOW_MAIN_INTERFACE") { this.showMainInterface(); return; }
    var choicesHtml = "";
...
GameUI.prototype.showQuestLog = function(quests) {
    ...
    qh += '</div>';
    this.setContent('<div class="quest-screen">' + this.renderHeader() + '<h2>📜 Quest Log</h2>' + qh + '<button class="btn btn-secondary" onclick="game.ui.showMainInterface()">Back</button></div>');
};
...
GameUI.prototype.showInventory = function(items) {
    ...
    invHtml += '</div>';
    this.setContent('<div class="inventory-screen">' + this.renderHeader() + '<h2>🎒 Inventory (' + items.length + '/' + c.maxInventory + ')</h2>' + eqHtml + invHtml + '<button class="btn btn-secondary" onclick="game.ui.showMainInterface()">Back</button></div>');
};
...
GameUI.prototype.showShop = function(shop) {
    ...
    ih += '</div>';
    this.setContent('<div class="shop-screen">' + this.renderHeader() + '<h2>🛒 ' + shop.name + '</h2>' + ih + '<button class="btn btn-secondary" onclick="game.ui.showMainInterface()">Leave</button></div>');
};
...
GameUI.prototype.showGamblingLocation = function(loc) {
    ...
    html += '</div><div class="gambling-stats"><p>Games Played: ' + st.gamesPlayed + '</p><p>Net: ' + (st.netProfit >= 0 ? '+' : '') + st.netProfit + ' gold</p></div><button class="btn btn-secondary" onclick="game.ui.showMainInterface()">Leave</button></div>';
