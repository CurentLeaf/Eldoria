...
Character.prototype.applyDifficultyMods = function() {
    if (this.difficulty === "hard") {
        this.baseStats.maxHealth = Math.floor(this.baseStats.maxHealth * 0.8);
        this.baseStats.health = this.baseStats.maxHealth;
    } else if (this.difficulty === "nightmare") {
        this.baseStats.maxHealth = Math.floor(this.baseStats.maxHealth * 0.6);
        this.baseStats.health = this.baseStats.maxHealth;
...
Character.prototype.getStartingSkills = function() {
    var skills = {
        warrior: {
            active: { id: "brutal_strike", name: "Brutal Strike", multiplier: 1.8, cooldown: 3, description: "A devastating blow that deals 180% damage." },
            passive: { id: "iron_skin", name: "Iron Skin", description: "Take 10% less damage from all sources." }
        },
        mage: {
            active: { id: "soul_burn", name: "Soul Burn", multiplier: 2.0, cooldown: 3, description: "Burn the enemy soul for 200% magic damage." },
            passive: { id: "arcane_mind", name: "Arcane Mind", description: "Intelligence increases critical hit chance." }
        },
        rogue: {
            active: { id: "backstab", name: "Backstab", multiplier: 2.5, cooldown: 2, description: "Strike from the shadows for 250% damage." },
            passive: { id: "evasion", name: "Evasion", description: "15% chance to dodge attacks completely." }
        }
    };
...
Character.prototype.takeDamage = function(amount, type) {
    ...
    if (this.characterClass === "warrior") {
        actualDamage = Math.floor(actualDamage * 0.9);
    }
...
Character.prototype.heal = function(amount) {
    var maxHealth = this.getEffectiveStat("maxHealth");
...
Character.prototype.fullHeal = function() {
    this.baseStats.health = this.baseStats.maxHealth;
    this.statusEffects = [];
    this.specialCooldown = 0;
};
...
Character.prototype.die = function() {
    ...
    if (!this.permadeath) {
        this.baseStats.health = Math.floor(this.baseStats.maxHealth * 0.25);
    }
...
Character.prototype.getFlag = function(flag) {
    return this.flags[flag];
};

Character.prototype.allocateStat = function(stat) {
    if (this.statPoints > 0) {
        this.baseStats[stat] = (this.baseStats[stat] || 0) + 1;
        this.statPoints--;

        if (stat === "vitality") {
            this.baseStats.maxHealth += 3;
            this.baseStats.health += 3;
        }
        return true;
    }
    return false;
